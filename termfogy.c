...
void termelo ( int semid, int *wr,float * tomb,FILE * fp )
{
    //termelo muvelet
	...
	if(semop(semid,termel,3)<0) syserr("termel")
        //kritikus szakasz
	if(semop(semid,&mutex,1)<0)
		syserr("mutex");
	
	if(semop(semid,&empty,1)<0)
		syserr("empty");
	
	if(semop(semid,&full,1)<0)
		syserr("full");
        //TODO: beirja  a szamot  a tombbe az iro index helyre 
	tomb[wr]=szam;
	wr++;
	
        //TODO: tovabbviszi az indexet, ha eleri a veget, akkor nullazza 

        //TODO: alszik MS*1000 mikroszekundumot
	sleep(MS*1000);
        //TODO: jelez muvelet
	if(semop(semid,jelez,1)<0) syserr("jelez");

	//  szem1=1  szem2=3 szem3=0   1 lepes termelox szem1=0 szem2=2 szem3=1
    ...
    //TODO: zarja  a fajlt es lecsatolj a memoriat
	fclose(fp);
...
void fogyaszto (int sor )
{
	...
	if(semop(semid,fogyaszt,3)<0) syserr("fogyaszto")
        //kritikus szakasz
	if(semop(semid,&mutex,1)<0)
		syserr("mutex");
    ...
    *rd=0;   //igy ez felesleges
    ...
    for (i=0; i < TERM ; i++) {
        ...
        if (pid<0)
            syserr("fork hiba");
        if ( pid == 0 ) {
            termelo ( semid,wr,arr,argv[1] ); //a termelo megkapja a sorszamat es a fajl nevet
            exit(EXIT_SUCCESS); 
        }
    }
