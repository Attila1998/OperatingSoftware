...
int main(int argc, char * argv[])
{
    ...
    // ahol a c opci=nak van egy paramétere is
    // az f1 f2 stb más paraméterek lehetnek, f1 az első ami
    // nem tartozik az opciókhoz  
    //
    // az extern int optopt - az utolsó beolvasott opció
    ...
    {
        switch(ret){
            case 'a': af=1;   // a opciót talált
                              //bejelöljük az af kapcsolót
                      break;
            case 'b': bf=1; 
                      break;
            case 'c': cf=1;    // a c opciónak kell paraméter is 
        ...
        if (af) printf ("a opció beállítva\n");
        if (bf) printf ("b opció beállítva\n");
        if (cf) printf ("c opció beállítva, paramétere: %s\n",cparam);
